...
namespace UnityEditor.AnimationWindowBuiltin
{
    class AnimationRecording
    {
        ...
        static bool HasAnyRecordableModifications(IAnimationRecordingState state, GameObject root, UndoPropertyModification[] modifications)
        {
            for (int i = 0; i < modifications.Length; i++)
            {
                if (AnimationUtility.PropertyModificationToEditorCurveBinding(modifications[i].previousValue, root, out _) != null)
                    return true;
            }
            return false;
        }
        ...
        static private void ProcessVector3Modification(IAnimationRecordingState state, EditorCurveBinding baseBinding, UndoPropertyModification modification, Transform target, string axis, float scale = 1.0f)
        {
            ...
            object currentValue = AnimationWindowUtility.GetCurrentValue(state.activeRootGameObject, binding);
        ...
        static public UndoPropertyModification[] Process(IAnimationRecordingState state, UndoPropertyModification[] modifications)
        {
            ...
            if (!HasAnyRecordableModifications(state, root, modifications))
                return modifications;
...
#pragma warning restore UAC2001
        ...
        {
            state.AddKey(binding, type, previousValue, currentValue);
        }
        ...
        static void AddRotationKey(IAnimationRecordingState state, EditorCurveBinding binding, Type type, Vector3 previousEulerAngles, Vector3 currentEulerAngles)
        {
            ...
            EditorCurveBinding[] additionalBindings = clip.RemapAnimationBindingForAddKey(binding);

            // Add key at current frame
            for (int i = 0; i < 3; i++)
                state.AddKey(additionalBindings[i], type, previousEulerAngles[i], currentEulerAngles[i]);
        }
            ...
            ref Dictionary<object, Vector3Modification> scaleModifications)
        {
            ...
            bool hasRootCurves = state.hasRootCurves;
            ...
            foreach (KeyValuePair<object, Vector3Modification> item in positionModifications)
            {
                ...
                if (isHumanBone)
                {
                    UnityEngine.Debug.LogWarning("Keyframing translation on humanoid rig is not supported!", target);
            ...
            foreach (KeyValuePair<object, RotationModification> item in rotationModifications)
            {
                ...
                if (isHumanBone)
                {
                    UnityEngine.Debug.LogWarning("Keyframing rotation on humanoid rig is not supported!", target);
            ...
            foreach (KeyValuePair<object, Vector3Modification> item in scaleModifications)
            {
                ...
                if (isHumanBone)
                {
                    UnityEngine.Debug.LogWarning("Keyframing scale on humanoid rig is not supported!", target);
