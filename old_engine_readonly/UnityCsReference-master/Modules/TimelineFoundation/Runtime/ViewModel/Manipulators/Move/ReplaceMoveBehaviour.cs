...
{
    class ReplaceMoveBehaviour : MoveBehaviour
    {
        ...
        protected override MoveManipulationResult TryInsert(InsertionParameters parameters)
        {
            ...
            ItemManipulator.ReplaceInsert(viewModel, parameters.destinationTracks, m_MoveItemsState, handler);
...
#pragma warning restore UAC2013
        ...
        protected override MoveStateBundle GetCurrentMoveStateBundle(bool updateState = false)
        {
            if (updateState)
                UpdateItemAndMarkerStatesFromContext();
            return new MoveStateBundle(m_MoveItemsState, m_MarkersState, false);
        }
        ...
        {
            for (var i = 0; i < context.manipulatedTracks.Count; i++)
            {
#pragma warning disable UAC2013 // Avoid Linq
                var command = new SetTrackContents(context.manipulatedTracks[i].track,
                    m_MoveItemsState.itemsDestinations?.ElementAtOrDefault(i),
                    m_MarkersState.markerDestinations?.ElementAtOrDefault(i));
#pragma warning restore UAC2013
                viewModel.Dispatch(command);
